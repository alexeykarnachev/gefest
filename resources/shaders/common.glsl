...
vec3 get_diffuse_color(vec3 normal, vec3 color, vec3 direction, float intensity) {
...
vec3 uv_to_sphere(float u, float v) {
    return uv_to_sphere(u, v, 1.0);
}

vec2 sphere_to_uv(vec3 pos) {
    float u = atan(pos.z, pos.x) / (2.0 * PI) + 0.5;
    float v = acos(pos.y / length(pos)) / PI;
    return vec2(u, v);
}
